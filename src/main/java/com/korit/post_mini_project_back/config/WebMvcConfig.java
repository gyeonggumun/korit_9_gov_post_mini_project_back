...
public class WebMvcConfig implements WebMvcConfigurer {
    ...
    public void addResourceHandlers(ResourceHandlerRegistry registry) {
        ...
        registry.addResourceHandler("/image/**")                // 외부에서 localhost:8080/image/** 의 요청이 들어오면
                .addResourceLocations("file:///" + uploadPath)  // 스프링 부트 서버 PC의 프로젝트 폴더 안의 upload 폴더로
                .resourceChain(true)                            // 연결
                .addResolver(new PathResourceResolver() {       // 요청 URL에 한글이 들어있을 수 있으니 익명 클래스 활용하여 한글 디코딩
                    @Override
